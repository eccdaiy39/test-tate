...
/**
 * Represents a 13-th extension field element. 
 */
typedef fp_t fp13_t[13];

/**
 * Represents a double-precision  13 extension field element.
 */
...
/**
 * Initializes a 13-th extension field with null.
 *
 * @param[out] A			- the 13-th extension element to initialize.
 */
#define fp13_null(A) do { fp13_do_1(fp_null, A); } while (0)

/**
 * Allocates a 13-th extension field element.
 *
 * @param[out] A			- the new 13-th extension field element.
 */
#define fp13_new(A) do { fp13_do_1(fp_new, A); } while (0)

/**
 * Frees an 13-th extension field element.
 *
 * @param[out] A			- the 13-th extension field element to free.
 */
#define fp13_free(A) do { fp13_do_1(fp_free, A); } while (0)

/**
 * Initializes a double-precision sextic extension field elements  with null.
 *
 * @param[out] A			- the 13-th extension field element to free.
 */
#define dv13_null(A) do { fp13_do_1(dv_null, A); } while (0)
 ...
 * @param[out] A			- the 13-th extension field element to free.
 */
#define dv13_new(A) do { fp13_do_1(dv_new, A); } while (0)

/**
 * Frees a double-precision 13-th extension field element.
 *
 * @param[out] A			- the 13-th extension field element to free.
 */
#define dv13_free(A) do { fp13_do_1(dv_free, A); } while (0)
 ...
 * @param[in] B				- the sparse dodecic extension field element.
 */
#define fp13_mul(C, A, B)	fp13_mul_lazyr(C, A, B)
 ...
 */
#if FPX_RDC == BASIC
#define fp13_sqr(C, A)	fp13_sqr_lazyr(C, A)
#elif FPX_RDC == LAZYR
#define fp13_sqr(C, A)	fp13_sqr_lazyr(C, A)
#endif
...
#define fp13_do_1(S, A) S(A[0]); S(A[1]); S(A[2]); S(A[3]); S(A[4]); S(A[5]); S(A[6]); S(A[7]); S(A[8]); S(A[9]); S(A[10]); S(A[11]); S(A[12])

#define fp13_do_2(S, C, A) S(C[0], A[0]); S(C[1], A[1]); S(C[2], A[2]); S(C[3], A[3]); S(C[4], A[4]); S(C[5], A[5]); S(C[6], A[6]); S(C[7], A[7]); S(C[8],\
A[8]); S(C[9], A[9]); S(C[10], A[10]); S(C[11], A[11]); S(C[12], A[12])


#define fp13_do_3(S, C, A, B) S(C[0], A[0], B[0]); S(C[1], A[1], B[1]); S(C[2], A[2], B[2]); S(C[3], A[3], B[3]); S(C[4], A[4], B[4]); S(C[5], A[5], B[5]); S(C[6], A[6], B[6]); S(C[7], A[7], B[7]); S(C[8], A[8], B[8]); S(C[9], A[9], B[9]); S(C[10], A[10], B[10]); S(C[11], A[11], B[11]); S(C[12], A[12], B[12])
...
/**
 * Assigns a random value to a 13-th extension field element.
 *
 * @param[out] a			- the 13-th extension field element to assign.
 */
void fp13_rand(fp13_t a);

/**
 * Initializes the 13-th extension field arithmetic module.
 */
void fp13_init();
 ...
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element to copy.
 */
void fp13_copy(fp13_t c, const fp13_t a);

/**
 * Assigns zero to a 13-th extension field element.
 *
 * @param[out] a			- the 13-th extension field element to zero.
 */
void fp13_zero(fp13_t a);

/**
 * Tests if a 13-th extension field element is zero or not.
 *
 * @param[in] a				- the 13-th extension field element to test.
 * @return 1 if the argument is zero, 0 otherwise.
 */
int fp13_is_zero(const fp13_t a);

/**
 * Prints a 13-th extension field element to standard output.
 *
 * @param[in] a				- the dodecic extension field element to print.
 */
void fp13_print(const fp13_t a);

int fp13_cmp_dig(const fp13_t a, dig_t b);

/**
 * Returns the result of a comparison between two 13-th extension field
 * elements.
 *
 * @param[in] a				- the first 13-th extension field element.
 * @param[in] b				- the second 13-th extension field element.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp13_cmp(const fp13_t a, const fp13_t b);

/**
 * Returns the result of a comparison between a 13-th extension field
 * element and a digit.
 *
 * @param[in] a				- the first 13-th extension field element.
 * @param[in] b				- the digit.
...
/**
 * Reads a 13-th extension field element from a byte vector in big-endian
 * format.
 * void fp13_read_str(fp13_t a,const char*str,int len,int radix);
 *
 * @param[out] a			- the result.
 * @param[in] bin			- the byte vector.
 * @param[in] len			- the buffer capacity.
...
/**
 * Assigns a 13-th extension field element to a digit.
 *
 * @param[in] a				- the dodecic extension field element.
 * @param[in] b				- the digit.
 */
...
/**
 * Adds two 13-th extension field elements. Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 13-th extension field element.
 * @param[in] b				- the second 13-th extension field element.
...
/**
 * Double two 13-th extension field elements. Computes c = 2a.
 *
 * @param[out] c			- the result.
...
/**
 * Substract two 13-th extension field elements. Computes c = a-b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 13-th extension field element.
 * @param[in] b				- the second 13-th extension field element.
...
/**
 * half a 13-th extension field element. Computes c = a/2.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the  13-th extension field element.
 */
...
/**
 * Negates a 13-th extension field element.
 *
 * @param[out] c			void fp13_mul_frb(fp13_t c,fp13_t a,int i);
- the result.
 * @param[out] a			- the 13-th extension field element to negate.
 */
void fp13_neg(fp13_t c, const fp13_t a);
/**
 * Reduces a 13-th extension element modulo the configured prime p.
 * Computes c = a mod p.
 *
 * @param[out] c			- the result.
...
/**
 * Multiples two 13-th extension field elements using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 * @param[in] b				- the 13-th extension field element.
 */
void fp13_mul_lazyr(fp13_t d, const fp13_t a, const fp13_t b);
/**
 * Computes the square of a 13-th extension field element using lazy
 * reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13th extension field element to square.
 */
void fp13_sqr_lazyr(fp13_t d, const fp13_t a);

/**
 * Inverts a 13-th extension field element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element to invert.
 */
void fp13_inv(fp13_t c, const fp13_t a);
/**
 * Inverts multiple 13-th extension field elements simultaneously.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the quadratic extension field elements to invert.
 * @param[in] n				- the number of elements.
 */
void fp13_inv_sim(fp13_t *c, const fp13_t *a, int n);

/**
 * Extracts the square root of a 13-th extension field element. Computes
 * c = sqrt(a). The other square root is the negation of c.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the extension field element.
...
int fp13_srt(fp13_t c, const fp13_t a);

int fp13_is_square(const fp13_t a);

/**
 * Computes a power of a 13-th extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the basis.
 * @param[in] b				- the exponent.
...
/**
 * Computes the Frobenius endomorphism of a 13-th extension element.
 * Computes c = a^(p^i).
 *
 * @param[out] c			- the result.
 * @param[in] a				- a 13-th extension field element.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp13_frb(fp13_t c, const fp13_t a,int i);

/**
 * Inverts a 13-th cyclotomic subgroup element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th cyclotomic subgroup element to invert.
 */
void fp13_inv_uni(fp13_t c, const fp13_t a);
