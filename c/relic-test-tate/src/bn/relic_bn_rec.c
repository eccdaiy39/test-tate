...
void bn_rec_frb_gt(bn_t* ki, const bn_t k, const bn_t x, const bn_t n) {
	int i;
	bn_t u[23];bn_t y[12]; bn_t v[12]; bn_t s[5];
	RLC_TRY {
		...
		for (i = 0; i <5; i++) {
    		...
    		bn_neg(s[0], x);
		bn_sub_dig(s[1], s[0], 1);
		bn_sqr(s[2], x);
		bn_sub_dig(s[3], s[2], 1);
		bn_sub(s[4], s[2], s[0]);
		bn_copy(u[1], x);
		for(i = 2; i<23; i++)bn_mul(u[i], u[i-1], x);
	
		bn_add(y[0], u[22], u[21]);
		bn_sub(y[0], y[0], u[19]);
		bn_sub(y[0], y[0], u[18]);
		bn_add(y[0], y[0], u[16]);
		...
		bn_add(y[3], u[16], u[15]);
		bn_sub(y[3], y[3], u[19]);
		bn_sub(y[3], y[3], u[18]);
		bn_sub(y[3], y[3], u[13]);
		bn_sub(y[3], y[3], u[12]);

		bn_add(y[4], u[15], u[14]);
		bn_add(y[4], y[4], u[8]);
		bn_add(y[4], y[4], u[7]);
		bn_sub(y[4], y[4], u[18]);
		bn_sub(y[4], y[4], u[17]);
		...
		bn_add(y[6], u[16], u[15]);
		bn_sub(y[6], y[6], u[13]);
		bn_sub(y[6], y[6], u[12]);

		bn_add(y[7], u[15], u[14]);
		bn_add(y[7], y[7], u[8]);
		bn_add(y[7], y[7], u[7]);
		bn_sub(y[7], y[7], u[12]);
		bn_sub(y[7], y[7], u[11]);
		...
		bn_add(y[9], u[13], u[12]);
		bn_neg(y[9], y[9]);

		bn_add(y[10], u[12], u[11]);
		bn_add(y[10], y[10], u[10]);
		bn_add(y[10], y[10], u[5]);
		bn_add(y[10], y[10], u[4]);
		bn_sub(y[10], y[10], u[8]);
		bn_sub(y[10], y[10], u[7]);
		bn_sub(y[10], y[10], u[2]);
		bn_sub(y[10], y[10], u[1]);

		bn_add(y[11], u[10], u[9]);
   		bn_add(y[11], y[11], u[4]);
    		bn_add(y[11], y[11], u[3]);
		bn_sub(y[11], y[11], u[7]);
		bn_sub(y[11], y[11], u[6]);
		bn_sub(y[11], y[11], u[1]);
		bn_sub_dig(y[11], y[11], 1);
		/*compute y[i]*k div n*/
		for(i = 0; i<12; i++){
			bn_mul(v[i], k, y[i]);
			bn_div(v[i], v[i], n);
    		...
    		bn_mul(ki[0], v[11], s[1]);
		bn_mul(ki[1], v[11], s[0]);
		bn_mul(ki[11], v[11], s[4]);

		bn_neg(ki[11], ki[11]);

		for(i = 2; i<11; i++)bn_copy(ki[i], ki[1]);

    		for(i = 0; i<10; i++)bn_add(ki[i], ki[i], v[10]);

		bn_mul(u[10],  v[10], s[3]);
		bn_neg(u[10], u[10]);
		bn_add(ki[10], ki[10], u[10]);

		bn_mul(u[11],  v[10], s[1]);

		bn_sub(ki[11], ki[11], u[11]);

		for(i = 0; i<10; i++){
			bn_mul(u[i], v[i], s[2]);
			bn_add(ki[i], ki[i], u[i]);
		}
  
		for(i = 1; i<11; i++){
			bn_mul(u[i], v[i-1], s[0]);
			bn_add(ki[i], ki[i], u[i]);
		}
		...
		ki[7]->used = 2;
	 	ki[4]->used = 2;
    		for(i = 2; i<12; i++){
			bn_add(ki[i], ki[i], v[i-2]);
		}
	
    		for(i = 1; i< 12; i++){bn_neg(ki[i], ki[i]);}

		bn_sub(ki[0], k, ki[0]);
    		for(i = 0; i<12; i++) ki[i]->used = 1;
		bn_add(ki[0], s[4], ki[0]);
		bn_add(ki[1], s[3], ki[1]);
		for(i = 2; i< 12; i++)bn_add(ki[i], s[2], ki[i]);
	}RLC_CATCH_ANY {
		RLC_THROW(ERR_CAUGHT);
	} RLC_FINALLY {
		for (i = 0; i < 12; i++) {
		...
		for (i = 0; i < 23; i++)bn_free(u[i]);
		for (i = 0; i < 5; i++)bn_free(s[i]);
