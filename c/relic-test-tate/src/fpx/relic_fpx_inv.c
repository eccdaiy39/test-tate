...
void fp13_inv(fp13_t c, const fp13_t a){
	...
	RLC_TRY{
		...
		fp13_mul(t0, t0, t1);
		fp_muln_low(u0, t0[1], a[12]);
		for(int i = 2; i<13; i++){
			fp_muln_low(u1, t0[i], a[13-i]);
			fp_addd_low(u0, u0, u1);		
		}
		fp_addd_low(u0, u0, u0);
		fp_muln_low(u1, t0[0], a[0]);
		fp_subc_low(u0, u1, u0);
		fp_rdc(t, u0);
		fp_inv(t, t);
		for(int i = 0; i<13; i++)fp_mul(t1[i], t, t0[i]);
		fp13_copy(c, t1);
...
void fp13_inv_sim(fp13_t *c, const fp13_t *a, int n) {
	int i;
	fp13_t u, *t = RLC_ALLOCA(fp13_t, n);
	...
	RLC_TRY {
		...
		for (i = 1; i < n; i++) {
			fp13_copy(t[i], a[i]);
			fp13_mul(c[i], c[i - 1], t[i]);
		}

		fp13_inv(u, c[n - 1]);

		for (i = n - 1; i > 0; i--) {
			fp13_mul(c[i], c[i - 1], u);
			fp13_mul(u, u, t[i]);
		}
		fp13_copy(c[0], u);
	}
